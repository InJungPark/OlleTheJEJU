...
public class SuggestDaoImpl implements SuggestDao{
	...
	@Override
	public List<SuggestDto> selectList_s(String search, int page) {
		List<SuggestDto> result = new ArrayList<SuggestDto>();
		int pageP = (page-1)*6;
		int pageN = page*6;
		System.out.println("pageP: "+pageP);
		...
		try {
			...
			System.out.println("[ERROR : SELECT_SUGGEST_SEARCH]");
			e.printStackTrace();
		}
	...
	@Override
	public int getAllCount_s(String search) {
		int cnt=0;
		String sql="countSearch";
		try {
			cnt = sqlSession.selectOne(NAMESPACE+sql, search);
		} catch (Exception e) {
			System.out.println("[ERROR : COUNT SEARCH]");
			e.printStackTrace();
		}
		return cnt;
	}
